--if exists (select 1 from dbo.sysobjects where id = object_id(N'CCHRWebApiProcGetPageData') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
--drop procedure CCHRWebApiProcGetPageData
--GO
CREATE  PROCEDURE [dbo].[CCHRWebApiProcGetPageData]
...
IF @CurrentPage = -1 --所有记录
BEGIN 
  SET @strsql = 'SELECT  '+@Fields
              + '  FROM ' + @TableName + ' ' + @strWhere + ' ORDER BY  '+ @Sort
END 
ELSE IF @CurrentPage = 1 --第一页提高性能
BEGIN 
