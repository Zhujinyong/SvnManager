<template>
  <div class="app-container">
     ...
     <el-input  style="width: 200px;" v-model="listQuery.domainAccount" placeholder="请输入域账号" clearable @change="resetPageIndex"></el-input>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogVisible" :close-on-click-modal="false">
      ...
      <el-input v-model="temp.domainAccount" placeholder="请输入"></el-input>
      </el-form-item>
...
<script>
import { getSvnUserList,addSvnUser,updateSvnUser } from '@/api/svn/user'
...
export default {
  ...
  methods: {
    ...
    save(){
      ...
      {
         addSvnUser(this.temp).then(response => {
      ...
      {
          updateSvnUser(this.temp).then(response => {
            this.dialogVisible=false
            this.$message({
             message: '修改成功',
             type: 'success'
    ...
    fetchData() {
      ...
      getSvnUserList(this.listQuery).then(response => {
